...
@section('content')
        ...
        <div class="max-w-7xl mx-auto px-4">
            ...
            <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
                <h1 class="text-3xl md:text-4xl font-bold text-[#0088cc] mb-4 md:mb-0 text-center md:text-left">
                    Data Pendidikan Dalam KK Desa Ganten
                </h1>
                <!-- Breadcrumbs -->
                <nav class="flex items-center space-x-2 text-sm bg-white rounded-lg shadow px-4 py-2" aria-label="Breadcrumb">
                    ...
                    <span class="text-gray-400">/</span>
                    <span class="text-gray-500 font-semibold">Data Pendidikan Dalam KK</span>
                </nav>
            </div>
                    ...
                    <div id="sidebarMenu" class="bg-white rounded-lg shadow-lg hidden lg:block">
                        ...
                        <div>
                            ...
                            <div class="py-2">
                                ...
                                <a href="{{ route('statistik.pendidikan') }}" class="sidebar-menu-item block px-4 py-2 text-gray-700 hover:bg-gray-100 hover:text-[#0088cc] transition-colors border-l-4 border-[#0088cc] bg-gray-100 text-[#0088cc] font-medium">
                ...
                <div class="lg:w-3/4">
                    ...
                    <div class="mb-6">
                        <div class="bg-white rounded-lg shadow-sm border-l-4 border-[#0088cc] p-4 mb-4">
                            <h2 class="text-lg font-bold text-gray-900">
                                <i class="fas fa-graduation-cap text-[#0088cc] mr-2"></i>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
                        ...
                        <div class="bg-white rounded-lg shadow-md p-6 border-l-4 border-indigo-500">
                            <div class="flex items-center">
                                <div class="flex-1">
                                    <h3 class="text-sm font-medium text-gray-600">Jenjang Pendidikan</h3>
                                    <p class="text-2xl font-bold text-gray-900">{{ count($dataPendidikan) }}</p>
                                </div>
                    ...
                    <!-- Chart Section - Pendidikan -->
                    <div id="pendidikan-section" class="bg-white rounded-lg shadow-lg p-6 mb-8">
                        ...
                        <div class="overflow-x-auto">
                            <table class="min-w-full divide-y divide-gray-200">
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    @foreach($dataPendidikan as $index => $pendidikan)
                                    <tr class="hover:bg-gray-50">
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{{ $index + 1 }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{{ $pendidikan['tingkat'] }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-center text-blue-600 font-semibold">{{ number_format($pendidikan['laki_laki'], 0, ',', '.') }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-center text-pink-600 font-semibold">{{ number_format($pendidikan['perempuan'], 0, ',', '.') }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-center font-bold text-gray-900">{{ number_format($pendidikan['jumlah'], 0, ',', '.') }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-center text-purple-600 font-semibold">
                                            {{ number_format(($pendidikan['jumlah'] / $totalPenduduk) * 100, 1) }}%
                                        </td>
                                    </tr>
                                    @endforeach
                                    <tr class="bg-gray-100 font-bold">
                                        <td colspan="2" class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">Total</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-center text-blue-800">{{ number_format(collect($dataPendidikan)->sum('laki_laki'), 0, ',', '.') }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-center text-pink-800">{{ number_format(collect($dataPendidikan)->sum('perempuan'), 0, ',', '.') }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-center text-gray-900">{{ number_format(collect($dataPendidikan)->sum('jumlah'), 0, ',', '.') }}</td>
                    ...
                    <div class="mt-8 text-center text-gray-500 text-sm bg-white rounded-lg shadow p-4">
                        ...
                        * Data tingkat pendidikan di atas merupakan data penduduk per tahun 2025 yang diperoleh dari database kependudukan Desa Ganten
                    </div>
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        function showChart(type) {
            ...
            // Create new chart
            pendidikanChart = new Chart(ctx, config);
        }
