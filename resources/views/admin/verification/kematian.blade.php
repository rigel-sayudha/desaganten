...
$breadcrumbs = [
    ...
    ['label' => 'Surat Keterangan Kematian', 'url' => '#'],
];
@endphp
        ...
        <div class="max-w-7xl mx-auto">
            ...
            <div class="mb-8">
                <div class="flex items-center justify-between">
                    <div>
                        <h1 class="text-3xl font-bold text-gray-900 mb-2">Verifikasi Surat Keterangan Kematian</h1>
                        <p class="text-gray-600">Detail verifikasi dan tahapan proses surat keterangan kematian</p>
                    </div>
                ...
                <div class="lg:col-span-2 space-y-6">
                    ...
                    <div class="bg-white rounded-xl shadow-lg p-6">
                        <div class="flex items-center justify-between mb-6">
                            ...
                            <div class="text-right">
                                <div class="text-3xl font-bold text-red-600">{{ $progress }}%</div>
                        ...
                        <div class="w-full bg-gray-200 rounded-full h-3 mb-6">
                            <div class="bg-gradient-to-r from-red-500 to-gray-600 h-3 rounded-full transition-all duration-500" 
                    ...
                    <div class="bg-white rounded-xl shadow-lg p-6">
                        <h2 class="text-xl font-bold text-gray-900 mb-6 flex items-center">
                            <i class="fas fa-cross text-red-500 mr-3"></i>
                            Data Pelapor & Almarhum
                        </h2>
                        
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <!-- Data Pelapor -->
                            <div class="md:col-span-2">
                                <h3 class="text-lg font-semibold text-gray-800 mb-4 border-b pb-2">Data Pelapor</h3>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Nama Pelapor</label>
                                <div class="p-3 bg-gray-50 rounded-lg border">{{ $surat->nama_pelapor }}</div>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">NIK Pelapor</label>
                                <div class="p-3 bg-gray-50 rounded-lg border">{{ $surat->nik_pelapor }}</div>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Hubungan dengan Almarhum</label>
                                <div class="p-3 bg-gray-50 rounded-lg border">{{ $surat->hubungan_pelapor }}</div>
                            </div>
                            <div>
                            ...
                            <div class="md:col-span-2">
                                <label class="block text-sm font-medium text-gray-700 mb-1">Alamat Pelapor</label>
                                <div class="p-3 bg-gray-50 rounded-lg border">{{ $surat->alamat_pelapor }}</div>
                            </div>
                            
                            <!-- Data Almarhum -->
                            <div class="md:col-span-2">
                                <hr class="my-4">
                                <h3 class="text-lg font-semibold text-red-700 mb-4">Data Almarhum</h3>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Nama Almarhum</label>
                                <div class="p-3 bg-red-50 rounded-lg border border-red-200">
                                    <strong class="text-red-800">{{ $surat->nama_almarhum }}</strong>
                                </div>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">NIK Almarhum</label>
                                <div class="p-3 bg-gray-50 rounded-lg border">{{ $surat->nik_almarhum }}</div>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Tempat, Tanggal Lahir</label>
                                <div class="p-3 bg-gray-50 rounded-lg border">
                                    {{ $surat->tempat_lahir_almarhum }}, {{ $surat->tanggal_lahir_almarhum ? $surat->tanggal_lahir_almarhum->format('d F Y') : '-' }}
                                </div>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Jenis Kelamin</label>
                                <div class="p-3 bg-gray-50 rounded-lg border">{{ $surat->jenis_kelamin_almarhum }}</div>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Agama</label>
                                <div class="p-3 bg-gray-50 rounded-lg border">{{ $surat->agama_almarhum }}</div>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Pekerjaan</label>
                                <div class="p-3 bg-gray-50 rounded-lg border">{{ $surat->pekerjaan_almarhum }}</div>
                            </div>
                            <div class="md:col-span-2">
                                <label class="block text-sm font-medium text-gray-700 mb-1">Alamat Almarhum</label>
                                <div class="p-3 bg-gray-50 rounded-lg border">{{ $surat->alamat_almarhum }}</div>
                            </div>
                            
                            <!-- Detail Kematian -->
                            <div class="md:col-span-2">
                                <hr class="my-4">
                                <h3 class="text-lg font-semibold text-gray-800 mb-4">Detail Kematian</h3>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Tanggal Kematian</label>
                                <div class="p-3 bg-gray-50 rounded-lg border">
                                    {{ $surat->tanggal_kematian ? $surat->tanggal_kematian->format('d F Y') : '-' }}
                                </div>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Waktu Kematian</label>
                                <div class="p-3 bg-gray-50 rounded-lg border">{{ $surat->waktu_kematian ?? '-' }}</div>
                            </div>
                            <div class="md:col-span-2">
                                <label class="block text-sm font-medium text-gray-700 mb-1">Tempat Kematian</label>
                                <div class="p-3 bg-gray-50 rounded-lg border">{{ $surat->tempat_kematian }}</div>
                            </div>
                            <div class="md:col-span-2">
                                <label class="block text-sm font-medium text-gray-700 mb-1">Sebab Kematian</label>
                                <div class="p-3 bg-gray-50 rounded-lg border">{{ $surat->sebab_kematian }}</div>
                            </div>
                            <div class="md:col-span-2">
    ...
    <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
        <div class="mt-3">
            ...
            <div class="mb-4">
                ...
                <textarea id="stageNotes" rows="3" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-red-500" placeholder="Tambahkan catatan untuk tahapan ini..."></textarea>
            </div>
            <div class="flex justify-end gap-3">
                <button onclick="closeModal()" class="px-4 py-2 bg-gray-300 text-gray-700 rounded-md hover:bg-gray-400 transition">
                    Batal
                </button>
                <button id="confirmButton" onclick="confirmUpdate()" class="px-4 py-2 bg-red-600 text-white rounded-md hover:bg-red-700 transition">
                    Update
                </button>
            </div>
        </div>
...
function confirmUpdate() {
    ...
    fetch(`/admin/verification/kematian/{{ $surat->id }}/stage/${currentStage}`, {
        method: 'POST',
        headers: {
