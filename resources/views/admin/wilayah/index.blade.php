...
<div class="flex">
    ...
    <main class="flex-1 ml-0 md:ml-64 min-h-screen bg-gray-50 pt-24 px-6">
            ...
            <table class="min-w-full divide-y divide-blue-200">
                ...
                <tbody class="bg-white divide-y divide-blue-100">
                    ...
                    <tr class="hover:bg-blue-50 transition">
                        ...
                        <td class="px-6 py-4 text-center text-gray-700">{{ $w->jumlah }}</td>
                        <td class="px-6 py-4 text-center text-blue-700 font-bold">{{ $w->laki_laki }}</td>
                        <td class="px-6 py-4 text-center text-pink-700 font-bold">{{ $w->perempuan }}</td>
                        <td class="px-6 py-4 text-center">
                            ...
                            <form action="{{ route('admin.wilayah.destroy', $w->id) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="text-red-600 font-bold" onclick="return confirm('Yakin hapus data?')">Hapus</button>
                            </form>
                        </td>
                    </tr>
                    @endforeach
        ...
        <div id="wilayahModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40 opacity-0 pointer-events-none transition-opacity duration-300">
